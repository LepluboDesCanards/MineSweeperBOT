...
class MineSweeper:
        ...
        if self.is_mouse_in_game():
            ...
            if pyxel.btnp(pyxel.MOUSE_BUTTON_LEFT):
                ...
                if self.mine_grid[tile[1]][tile[0]] == 1:
                    ...
                    if tile not in self.reveled_tiles: self.reveled_tiles.append(tile)
    ...
    def how_many_mines_around_me(self, tile:tuple[int, int]):

        neigbors = [(x, y) for x in range(tile[0]-1, tile[0]+2) for y in range(tile[1]-1, tile[1] + 2) if (0 <= x < len(self.mine_grid)) and (0 <= y < len(self.mine_grid[0])) and ((x, y) != tile) and (self.mine_grid[y][x] == 1)]
        return len(neigbors), neigbors
